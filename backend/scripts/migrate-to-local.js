/**
 * Migrate database records in MongoDB Atlas from remote URLs (dudhrejvadwala.com)
 * back to local /uploads/... paths.
 */
...
function toLocalPath(remoteUrl) {
  ...
  if (!remoteUrl.includes('dudhrejvadwala.com')) return remoteUrl;
...
async function run() {
  ...
  if (settings) {
    ...
    if (changed) {
      await settings.save();
      updatedSettings = 1;
      console.log('✅ Settings updated to local paths');
    }
  }

  // 2. Banners
  const banners = await Banner.find({ image: /dudhrejvadwala\.com/ });
  ...
  // 3. History Sections
  const history = await HistorySection.find({ image: /dudhrejvadwala\.com/ });
  ...
  // 4. Gallery Items
  const gallery = await GalleryItem.find({ image: /dudhrejvadwala\.com/ });
  ...
  // 5. Activities
  const activities = await Activity.find({ image: /dudhrejvadwala\.com/ });
  ...
  // 6. Festivals
  const festivals = await Festival.find({ image: /dudhrejvadwala\.com/ });
  ...
  // 7. Payment Info
  const payment = await PaymentInfo.find({ $or: [{ qrImage: /dudhrejvadwala\.com/ }, { qrCode: /dudhrejvadwala\.com/ }] });
  for (const item of payment) {
    let changed = false;
    if (item.qrImage && item.qrImage.includes('dudhrejvadwala.com')) {
      item.qrImage = toLocalPath(item.qrImage);
      changed = true;
    }
    if (item.qrCode && item.qrCode.includes('dudhrejvadwala.com')) {
  ...
  for (const item of gaushalas) {
    ...
    if (item.photos && item.photos.length > 0) {
      item.photos = item.photos.map((p) => {
        if (p.src && p.src.includes('dudhrejvadwala.com')) {
          const originalSrc = p.src;
          const localSrc = toLocalPath(p.src);
          if (originalSrc !== localSrc) {
            changed = true;
            return { ...p.toObject(), src: localSrc };
          }
        }
        return p;
      });
    }
    if (item.heroImage && item.heroImage.includes('dudhrejvadwala.com')) {
      const originalHero = item.heroImage;
      item.heroImage = toLocalPath(item.heroImage);
  ...
  console.log('\n═══════════════════════════════════════');
  console.log('  Migration back to Local Paths Complete!');
  console.log('═══════════════════════════════════════');
