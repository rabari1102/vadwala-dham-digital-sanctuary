...
const Festivals = () => {
  ...
  useEffect(() => {
    api.getFestivals().then((d) => { if (d?.length) setFestivals(d); });
  }, []);
    ...
    <section id="festivals" className="py-16 md:py-24 bg-section-alt">
      <div className="max-w-7xl mx-auto px-4 sm:px-6" ref={containerRef}>
        <div className="fade-in-section text-center mb-12">
          <span className="inline-block bg-orange-100 text-orange-600 text-xs font-semibold tracking-widest uppercase px-4 py-1.5 rounded-full mb-3">Celebrations</span>
          <h2 className="section-heading">પર્વ ઉજવણી</h2>
          <p className="section-sub">જન્માષ્ટમી, દિપાવલી, હોળી-ધૂળેટી અને ગુરૂ પૂર્ણિમા</p>
          <LotusDivider />
        </div>

        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 max-w-5xl mx-auto mb-10">
          {festivals.map((f, i) => (
            <div key={f._id || i} className="fade-in-section hover-lift rounded-2xl overflow-hidden shadow-md group" style={{ transitionDelay: `${i * 100}ms` }}>
        ...
        {upcoming.length > 0 && (
          <div className="fade-in-section max-w-2xl mx-auto">
            <h3 className="font-heading font-bold text-xl text-gray-800 text-center mb-5">📅 આવનાર ઉત્સવ ની યાદી</h3>
            <div className="space-y-3">
              {upcoming.map((u, i) => (
                <div key={u._id || i} className="hover-lift flex items-center gap-4 rounded-xl border border-orange-100 bg-white px-5 py-4 shadow-sm">
                  <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-orange-50 text-2xl">{u.icon}</div>
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <span className="inline-block rounded-full bg-orange-100 text-orange-600 text-xs font-semibold px-3 py-0.5 mt-1">Upcoming</span>
                  </div>
                </div>
              ))}
            </div>
