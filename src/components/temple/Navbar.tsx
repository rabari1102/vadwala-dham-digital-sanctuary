...
const navLinks = [
  { label: 'Home',      labelGuj: 'હોમ',      href: '/'          },
  { label: 'About',     labelGuj: 'ઇતિહાસ',   href: '/about'     },
  { label: 'Events',    labelGuj: 'ઉત્સવ',    href: '/events'    },
  { label: 'Gallery',   labelGuj: 'ગેલેરી',   href: '/gallery'   },
  { label: 'Donations', labelGuj: 'દાન',       href: '/donations' },
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 py-3">
          ...
          <Link to="/" className="flex items-center gap-2.5 min-w-0">
            <div className={`flex-shrink-0 flex h-9 w-9 items-center justify-center rounded-full text-lg font-bold shadow transition-all duration-300 ${
              scrolled || !isHome ? 'bg-orange-500 text-white' : 'bg-white/20 text-white backdrop-blur-sm'
            }`}>
              🕉️
            </div>
            <div className="leading-tight min-w-0">
              <span className={`block text-sm font-heading font-bold truncate transition-colors duration-300 ${
                scrolled || !isHome ? 'text-orange-600' : 'text-white drop-shadow'
              }`}>
                શ્રી વડવાળા મંદિર
              </span>
              <span className={`block text-[10px] truncate transition-colors duration-300 ${
                scrolled || !isHome ? 'text-gray-500' : 'text-white/75 drop-shadow'
              }`}>
          ...
          <button
            className={`lg:hidden p-2 rounded-xl transition-colors ${
              scrolled || !isHome ? 'text-gray-700 hover:bg-orange-50' : 'text-white hover:bg-white/10'
            }`}
